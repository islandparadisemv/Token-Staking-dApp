...
const Staking = (props) => {
  ...
  return (
    <div className={classes.Staking}>
      ...
      <h1> Island Paradise $IP Staking dApp</h1>
      <p>{props.account}</p>
      <h3>
      ...
      <div className={classes.totals}>
        <h4>
          Total Staked (by all users): {props.totalStaked} Island Paradise Token ($IP)
        </h4>
        <div>&nbsp;</div>
        <h5>My Stake: {props.myStake} Island Paradise Token ($IP) </h5>
        <h5>
          My Estimated Reward:{' '}
          {((props.myStake * props.apy) / 36500).toFixed(3)} Island Paradise Token ($IP)
        </h5>
        <h5 onClick={goMax} className={classes.goMax}>
          My balance: {props.userBalance} Island Paradise Token ($IP)
        </h5>
      </div>
    </div>
  );
